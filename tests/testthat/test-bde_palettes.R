test_that("Test error", {
  expect_snapshot(bde_palettes(palette = "none"),
    error = TRUE
  )
})
...
test_that("Max value", {
  ...
  expect_snapshot(nmore <- bde_palettes(n = 23))
...
test_that("Switch pal", {
  ok <- bde_palettes(n = 6)
  other <- bde_palettes(n = 6, palette = "bde_rose_pal")
...
test_that("test alpha", {
  ok <- bde_palettes(n = 6)
  ok3 <- bde_palettes(n = 6, alpha = 0.2)
