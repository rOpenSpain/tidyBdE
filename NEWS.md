...
- Empty-result and download messages are now more specific, omit long query URLs
  and avoid implying that all BdE resources are unavailable.
- `bde_series_api_latest()` and `bde_series_api_load()` now report malformed or
  incomplete API responses with clear errors.
...
- Added `vignette("csv_manual", package = "tidyBdE")`, which explains the
...
- `?bde_ind_db` was updated to include the field `Nombre_de_la_serie` (API
